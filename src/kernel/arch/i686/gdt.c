...
typedef struct {
    ...
    uint8_t FlagsLimitHi; // limit flags (bits 16-19) 
    uint8_t BaseHi; // base (bits 24-31)
} __attribute__((packed)) GDTEntry;

typedef struct {
    uint16_t Limit; // size gdt - 1
    GDTEntry* Address;
} __attribute__((packed)) GDTDescriptor;

typedef enum {
    GDT_ACCESS_CODE_READABLE = 0x02, // read and write access
    GDT_ACCESS_CODE_WRITABLE = 0x02,

    GDT_ACCESS_CODE_CONFORMING = 0x04, // code execution privilege
    GDT_ACCESS_DATA_DIRECTION_NORMAL = 0x00, // code execution direction
    GDT_ACCESS_DATA_DIRECTION_DOWN = 0x04,

    GDT_ACCESS_DATA_SEGMENT = 0x10, // executable or not
    GDT_ACCESS_CODE_SEGMENT = 0x18,

    GDT_ACCESS_DESCRIPTOR_TSS = 0x00, // task state segment

    GDT_ACCESS_RING0 = 0x00, // privilige level 
    GDT_ACCESS_RING1 = 0x20,
    GDT_ACCESS_RING2 = 0x40,
    GDT_ACCESS_RING3 = 0x60,
    
    GDT_ACCESS_PRESENT = 0x80 // present or not (enable/disable)
} GDT_ACCESS;

typedef enum {
    GDT_FLAG_64BIT = 0x20, // size flag
    GDT_FLAG_32BIT = 0x40,
    GDT_FLAG_16BIT = 0x00,

    GDT_FLAG_GRANULARITY_1B = 0x00, // granularity
...
#define GDT_BASE_HI(base) ((base >> 24) & 0xFF)
