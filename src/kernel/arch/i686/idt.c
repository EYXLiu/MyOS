...
typedef struct {
    uint16_t BaseLow; // offset low (bits 0-15)
    uint16_t SegmentSelector;
    uint8_t Reserved;
    uint8_t Flags;
    uint16_t BaseHi; // offset high (bits 16-31)
...
void __attribute__((cdecl)) i686_IDT_Load(IDTDescriptor* idtDescrptor);
...
void i686_IDT_SetGate(int interrupt, void* base, uint16_t segmentDescriptor, uint8_t flags) {
    ...
    g_IDT[interrupt].BaseHi = ((uint32_t)base >> 16) & 0xFFFF;
}
