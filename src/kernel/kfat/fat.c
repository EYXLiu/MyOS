...
uint32_t FS_Load() {
    ...
    log_debug("FS", "filesystem loaded; total blocks: %u", g_Superblock.total_blocks);
...
void FS_FileDelete(Directory* parent, const char* name) {
    ...
    for (int i = 0; i < parent->count; i++) {
        ...
        if (buffer[0] == FILE_TYPE_FILE) {
            FileEntry candidate;
            memcpy(&candidate, buffer, sizeof(FileEntry));
            if (memcmp(candidate.name, name, strlen(name) + 1) == 0) {
                FS_BatClear(candidate.block);
                FS_BatSave(candidate.block);
                ...
                memset(buffer, 0, sizeof(buffer));
                memcpy(buffer, parent, sizeof(FileEntry));
                FS_WriteBlock(parent->block, buffer);
                return;
            }
        }
...
void FS_FileBlockRead(uint32_t block, uint8_t* buffer, uint32_t bytes) {
    while (block != 0xFFFFFFFF && bytes > 0) {
        uint8_t buf[FS_BLOCK_SIZE] = {0};
        FS_ReadBlock(block, buf);
        FileHeader fh;
        memcpy(&fh, buf, sizeof(FileHeader));
        size_t copy = fh.size > bytes ? bytes : fh.size;
        memcpy(buffer, fh.data, copy);
