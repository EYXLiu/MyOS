...
arch::i686::VGATextDevice::VGATextDevice() : m_ScreenX(0), m_ScreenY(0) { }

size_t arch::i686::VGATextDevice::Read(uint8_t* data, size_t size) {
    return 0;
}

size_t arch::i686::VGATextDevice::Write(const uint8_t* data, size_t size) {
...
void arch::i686::VGATextDevice::Clear() {
...
void arch::i686::VGATextDevice::PutChar(int x, int y, char c) {
    g_ScreenBuffer[2 * (y * ScreenWidth + x)] = c;
}

void arch::i686::VGATextDevice::PutColor(int x, int y, uint8_t color) {
    g_ScreenBuffer[2 * (y * ScreenWidth + x) + 1] = color;
}

char arch::i686::VGATextDevice::GetChar(int x, int y) {
    return g_ScreenBuffer[2 * (y * ScreenWidth + x)];
}

uint8_t arch::i686::VGATextDevice::GetColor(int x, int y) {
    return g_ScreenBuffer[2 * (y * ScreenWidth + x) + 1];
}

void arch::i686::VGATextDevice::SetCursor(int x, int y) {
...
void arch::i686::VGATextDevice::Scrollback(int lines) {
...
void arch::i686::VGATextDevice::PutChar(char c) {
    switch (c) {
        case '\n':
            m_ScreenX = 0;
            m_ScreenY++;
